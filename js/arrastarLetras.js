    ...
    let html = `
      <div class="fade-in" style="max-width:500px; margin:0 auto;">
        ...
        <div style="min-height:60px; border:2px dashed var(--border); border-radius:8px; margin:16px 0; 
          ...
          ${ArrastarLetras.resposta.map(l => `<span data-action="ArrastarLetras.removerLetra" data-args='[${l.id}]' style="background:var(--primary); color:white; padding:8px 12px; border-radius:4px; cursor:pointer;">${l.letra}</span>`).join('')}
        </div>
        <div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:center;">`;
    ArrastarLetras.letrasDisponiveis.forEach(item => {
      if (!item.usada) html += `<button class="btn-secundario" style="font-size:1.5rem; padding:8px 16px;" data-action="ArrastarLetras.adicionarLetra" data-args='[${item.id}]'>${item.letra}</button>`;
      else html += `<button class="btn-secundario" disabled style="opacity:0.5;">${item.letra}</button>`;
    });
    html += `</div>
        <div style="margin-top:16px; display:flex; gap:8px;">
          <button class="btn-perigo" data-action="ArrastarLetras.limpar">Limpar</button>
          <button class="btn-primario" data-action="ArrastarLetras.verificar">Verificar</button>
