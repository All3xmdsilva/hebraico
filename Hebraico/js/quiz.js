const Quiz = {
  ...
  montarQuestao() {
    ...
    q.opcoes.forEach((op, i) => {
      ...
      html += `<button class="${classe}" data-action="Quiz.selecionarOpcao" data-args='[${i}]' ${Quiz.selecionada !== null ? 'disabled' : ''}>${op}</button>`;
    });
    html += `</div>`;
    if (Quiz.selecionada !== null) {
      ...
      html += `<button class="btn-primario" data-action="Quiz.proxima">Próxima</button>`;
    }
    html += `</div>`;
    return html;
  },
  ...
  selecionarOpcao(indice) {
    ...
    if (indice === q.correta) {
      ...
      Storage.atualizarMissao('quiz', 1);
